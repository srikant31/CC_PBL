...
# Configure Content Security Policy to allow inline styles and external Bootstrap
# Doing this ensures that the index.html loads without breaking styling
csp = {
    'default-src': [
        '\'self\'',
    ],
...
# Talisman automatically forces HTTPS and sets security headers like X-Frame-Options, X-XSS-Protection, HSTS, etc.
...
def generate_improvement_tips(analysis_results):
    ...
    if analysis_results.get('unused_variables'):
        tips.append("💡 Remove unused variables to clean up your code")
...
def run_security_scan(code):
    ...
    try:
        ...
        # Locate bandit relative to the current Python interpreter (works inside venv)
        scripts_dir = _os.path.dirname(sys.executable)
        bandit_exe = _os.path.join(scripts_dir, "bandit.exe" if sys.platform == "win32" else "bandit")

        if not _os.path.isfile(bandit_exe):
            raise ImportError("bandit CLI not found in venv")
        ...
        result = subprocess.run(
            [bandit_exe, "-q", "-f", "txt", temp_path],
            capture_output=True, text=True, timeout=15
        )

        # Bandit returns exit code 1 when it finds issues, output on stdout
        for line in result.stdout.splitlines():
            line = line.strip()
            if line.startswith(">>") or "Issue:" in line or "Severity:" in line:
                security_issues.append(line)

        if not security_issues:
            security_issues.append("Bandit: No high-severity issues detected.")

    except ImportError:
        security_issues.append("Bandit not installed for advanced security scanning")
    except Exception as e:
        security_issues.append(f"Bandit error: {str(e)}")
    finally:
...
@app.route("/debug")
def debug():
    """Information Disclosure vulnerability for educational testing"""
    import os
    return f"<h1>[SECURITY VULNERABILITY] Debug Info</h1><pre>{str(os.environ)}</pre>"
