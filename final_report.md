...
*   **Static Security Analysis:** Findings from Bandit AST scanning.
...
*   **Understanding 3 (Security):** To mandate security, we utilize **Bandit** (a dedicated Python AST security scanner) with custom Regular Expressions to identify sensitive vulnerability patterns (e.g., catching `subprocess.run(shell=True)`).
...
3.  **Application Security:** Validate that the Bandit analyzer successfully catches Command Injection and other malicious code vectors in the submitted code.
...
| **Level 3 (Security)** | **Static Application Security Testing (SAST):** Static source code analysis of user inputs. | **Bandit** & Custom Regex | Scanning the submitted AST for sensitive patterns, code execution risks, and injection vulnerabilities. |
...
| **TC-002** | **Validate Security Warning Triggers:** Adversarial attempt to inject shell commands via subprocess. | - Flask server running. | 1. Navigate to application.<br>2. Input malicious code: `import subprocess; subprocess.run(['ls'], shell=True)`.<br>3. Check 'Security Scan' option.<br>4. Click 'Analyze Code'. | **Bandit** intercepts the payload. UI displays `"Security Issues:"` and `"Shell Injection"`. | System flagged subprocess call and rendered contextual warning. | **PASS** |
...
| **TC-011** | **Combinatorial Hybrid Analysis:** Mixed valid + malicious code vectors. | - Hybrid SAST active. | Input valid function alongside an `eval()` exploit. | System parses valid function but triggers security warning for `eval`. | Both logical scoring and security alerting active. | **PASS** |
...
| **TC-002** | Injection Security | SAST | **Bandit** |
...
| **TC-011** | Hybrid Analysis | Combinatorial Testing | Hybrid SAST + Manual |
...
*   **TC-011 (Hidden Viruses):** *The Trojan Horse Test.* We hid a dangerous virus inside a perfectly normal function to prove our security scanner checks *every single line*, not just the top layer.
...
| **BUG-02** | **XSS Vulnerability** | **Critical** | **Manual Review** | Submit `<script>alert(1)</script>` | Script should be escaped | Script executed in browser | **Identified via internal review.** Resolved by enabling Jinja2 auto-escaping. | **Fixed** |
...
*   **Script:** `selenuim test_v2.py` (**9 Success/Failure Scenarios**)
*   **Unique UI Test Cases Covered:**
    ...
    7.  `test_improvement_tips_logic` — Verifies concurrent display of multiple optimization tips.
    8.  `test_form_state_persistence` — Verifies checkbox stability after form submission.
    9.  `test_malformed_input_graceful_failure` — Verifies resilience against non-Python gibberish.
...
*   **Description:** Built-in auditing to detect high-severity vulnerabilities in submitted Python code.
...
2.  **Fuzz Testing (111):** 111 automated randomized iterations using the Hypothesis framework.
3.  **UI/E2E Scenarios (9):** Specific browser-level interactions via Selenium WebDriver.
...
| 2 | **Fuzz Testing** | Black-Box | **Hypothesis** | `test_fuzz.py` | 111 |
| 3 | **E2E UI Testing** | Black-Box | **Selenium** | `selenuim test_v2.py` | 9 |
